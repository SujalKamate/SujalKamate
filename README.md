<h1 align="center">
 
  <img src="https://raw.githubusercontent.com/Tarikul-Islam-Anik/Animated-Fluent-Emojis/master/Emojis/Computers/Desktop%20Computer.png" width="40" /> 
  SUJAL KAMATE | AI SYSTEMS ENGINEER
</h1>

<p align="center"><h1 align="center">
  <img src="https://raw.githubusercontent.com/Tarikul-Islam-Anik/Animated-Fluent-Emojis/master/Emojis/Computers/Desktop%20Computer.png" width="40" />
  SUJAL KAMATE | AI SYSTEMS ENGINEER
</h1>

<p align="center">
  <a href="https://linkedin.com/in/sujal-kamate-66391a2ba">
    <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white"/>
  </a>
  <a href="mailto:sujalvkamate@gmail.com">
    <img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white"/>
  </a>
  <a href="https://leetcode.com/u/EKjmM13shb/">
    <img src="https://img.shields.io/badge/LeetCode-FFA116?style=for-the-badge&logo=leetcode&logoColor=black"/>
  </a>
</p>

<p align="center">
<img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&size=22&duration=3000&pause=1000&color=00F7FF&center=true&vCenter=true&width=800&lines=Architecting+Agentic+AI+Workflows;Specializing+in+Hybrid+RAG+Systems;Intel+Unnati+Industrial+Trainee;AI+Domain+Lead+@+ACM+NMIET" />
</p>

---

# ⚙️ Engineering Profile

- 🎓 **2nd Year B.E. Computer Engineering** @ PCET’s NMIET Pune (CGPA **8.83**)  
- 🤖 Specializing in **Agentic AI**, **Generative AI**, and **Hybrid RAG Systems**  
- 🧠 AI Domain Lead @ ACM NMIET | Technical Lead @ GDG NMIET  
- 🏆 **5× Hackathon Finalist** across 50+ competitions including Mumbai Hacks & Project Morphus  
- 🚀 Selected for **Intel® Unnati Industrial Training Program 2025**

---

# 🧠 Neural Projects (Selected Work)

## 🤖 Intel Unnati – Multilingual NCERT Hybrid RAG
- Built Hybrid RAG system with **dense semantic retrieval (multilingual-e5)** + **TF-IDF sparse search** using ChromaDB  
- Multimodal pipeline supporting **Text + OCR (OpenCV/Tesseract) + Voice (Whisper)**  
- Grounded answer generation using **Mistral via Ollama** with MMR re-ranking  

## 🌾 AGRIXAI – Modular Agentic Agriculture Platform
- YOLOv8 crop disease detection system  
- Agentic AI planning using LangChain  
- FAISS-based knowledge retrieval from government agriculture documents  

## 🛡️ SENTINEL-X – Proactive Cyber Threat Prediction
- Behaviour-based cybersecurity engine using system log analysis  
- Validated attacks using Kali Linux tools (Nmap & Hydra)  

## 🌍 TRINETRA – Geological Monitoring System
- FastAPI + Next.js system for real-time landslide risk prediction  
- Sensor simulation for rainfall, seismic, and displacement monitoring  

## 🧠 Learner AI – Mind-Map Learning Assistant
- AI system converting study material into structured learning mind maps  

---

# 🛠️ Technical Arsenal

### 🤖 AI / ML
LangChain • HuggingFace • TensorFlow • PyTorch • Scikit-learn • NLP  

### ⚡ Generative AI & RAG
Agentic AI • Hybrid RAG • ChromaDB • FAISS • Mistral • Ollama • Gemini AI  

### 💻 Languages & Databases
Python • C++ • Java • JavaScript • SQL • PostgreSQL • MySQL  

### 🌐 Backend & DevOps
FastAPI • Node.js • Docker • Git • Vercel • Render • Railway  

---

# 🏆 Key Achievements
- ⭐ Selected for **Intel Unnati AI Industrial Training**  
- ⭐ **5× Hackathon Finalist** across major competitions  
- ⭐ Organized & mentored **50+ technical workshops**  
- ⭐ Led AI training sessions for **500+ students**  

---

# 📊 System Stats
<p align="center">
<img src="https://stats.dooboo.io/api/github-stats-advanced?login=SujalKamate&theme=dark"/>
</p>

---

# ⚡ System Status
- 🔭 Currently building advanced **Agentic AI automation workflows**
- 💬 Ask me about **RAG systems, vector databases, and autonomous agents**
- 📫 Secure uplink → **sujalvkamate@gmail.com**

---

<p align="center">
<b>🚀 Looking for AI Internships, Research Opportunities & Hackathon Teams</b>
</p>
  <a href="https://linkedin.com/in/sujal-kamate-66391a2ba"><img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white"/></a>
  <a href="mailto:sujalvkamate@gmail.com"><img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white"/></a>
  <a href="https://leetcode.com/u/EKjmM13shb/"><img src="https://img.shields.io/badge/LeetCode-FFA116?style=for-the-badge&logo=leetcode&logoColor=black"/></a>
</p>

<p align="center">
<img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&size=22&duration=3000&pause=1000&color=00F7FF&center=true&vCenter=true&width=800&lines=Architecting+Agentic+AI+Workflows;Specializing+in+Hybrid+RAG+Systems;Intel+Unnati+Industrial+Trainee;AI+Domain+Lead+@+ACM+NMIET" />
</p>

---

## ⚙️ Engineering Profile
* [cite_start]**Specialization**: Building scalable, intelligent systems focused on **Agentic AI**, **Generative AI**, and **Retrieval-Augmented Generation (RAG)**[cite: 9, 12, 16].
* **Academic Foundation**: 2nd Year B.E. [cite_start]Computer Engineering @ **PCET’s NMIET Pune** with a **CGPA of 8.83**[cite: 9, 56, 57].
* [cite_start]**Technical Leadership**: Serving as the **AI Domain Lead** for the ACM Student Chapter and **Technical Lead** for GDG NMIET[cite: 81, 86].
* [cite_start]**Hackathon Excellence**: **5x Finalist** across 50+ national-level competitions, including **Mumbai Hacks** and **Project Morphus**[cite: 11, 68, 69, 95].

---

## 🧠 Neural Projects (Selected Works)

### 🤖 Intel Unnati: Multilingual NCERT Hybrid RAG
[cite_start]*Industrial training project developed under the guidance of Dr. Renuka Goud and Dr. Renuka Kajale[cite: 20, 24].*
- [cite_start]**Architecture**: Implemented a **Hybrid RAG** system using **dense semantic retrieval** (multilingual-e5) and **sparse TF-IDF search** via **ChromaDB**[cite: 25, 26, 31].
- [cite_start]**Multimodal Pipeline**: Supports text, **OCR** (OpenCV + Tesseract), and **Voice queries** (Whisper speech-to-text)[cite: 27, 33].
- [cite_start]**Engine**: Answer generation using **Mistral LLM** via **Ollama** with strict context-grounding and MMR re-ranking[cite: 28, 34].



### 🌾 AGRIXAI: Modular Agentic Platform
- [cite_start]**Vision Layer**: Utilizes **YOLOv8** for real-time crop disease detection[cite: 40].
- [cite_start]**Reasoning Layer**: Integrates **Agentic AI planning** with **LangChain** for autonomous agricultural decision-making[cite: 40].
- [cite_start]**Knowledge Base**: Employs **FAISS vector retrieval** from government documents to ensure trusted recommendations[cite: 41].

### 🛡️ SENTINEL-X: Proactive Threat Anticipation
- [cite_start]**Logic**: Behavioral-based cybersecurity system designed to predict threats through system log analysis[cite: 51].
- [cite_start]**Validation**: Conducted attack simulations using **Kali Linux** tools (Nmap/Hydra) to validate anomaly detection accuracy[cite: 52, 53].

---

## 🛠️ Technical Arsenal

| Category | Technologies |
| :--- | :--- |
| **AI / Machine Learning** | [cite_start]LangChain, Hugging Face, TensorFlow, PyTorch, Scikit-learn, NLP [cite: 15, 16] |
| **Generative AI & RAG** | [cite_start]Agentic AI, RAG, ChromaDB, FAISS, Mistral, Ollama, Gemini AI [cite: 16, 28, 40, 48] |
| **Languages & Databases** | [cite_start]Python, C++, Java, JavaScript, SQL, PostgreSQL, MySQL [cite: 14, 17] |
| **DevOps & Backend** | [cite_start]FastAPI, Docker, Git, Node.js, Vercel, Railway [cite: 15, 18] |

---

## 🏆 Key Achievements
* [cite_start]**Intel® Unnati 2025**: Selected for high-performance industrial training in AI development[cite: 20, 97].
* [cite_start]**Community Impact**: Organized and mentored **50+ technical workshops** and **10+ hackathon teams**[cite: 83, 84, 99].
* [cite_start]**Leadership**: Serving as **AI Domain Lead** (ACM) and **Technical Lead** (GDG), driving technical excellence for 500+ students[cite: 83, 88].

---

## ⚡ System Status
* 🔭 **Current Project**: Optimizing Agentic Workflows for automated code review.
* 💬 **Ask Me About**: RAG Architectures, Vector Embeddings, and Autonomous Agents.
* 📫 **Secure Uplink**: `sujalvkamate@gmail.com`

---
<p align="center">
  <b>Looking for AI Engineering Internships and Research Collaborations.</b>
</p>
</h1>

<p align="center">
  <a href="https://linkedin.com/in/sujal-kamate-66391a2ba"><img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white"/></a>
  <a href="mailto:sujalvkamate@gmail.com"><img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white"/></a>
  <a href="https://leetcode.com/u/EKjmM13shb/"><img src="https://img.shields.io/badge/LeetCode-FFA116?style=for-the-badge&logo=leetcode&logoColor=black"/></a>
</p>

<p align="center">
<img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&size=22&duration=3000&pause=1000&color=00F7FF&center=true&vCenter=true&width=800&lines=CORE:+Agentic+AI+|+RAG+Architect;STACK:+FastAPI+|+LangChain+|+PyTorch;OPS:+Intel+Unnati+Trainee+|+ACM+AI+Lead;STATUS:+Seeking+AI+Research+Internships" />
</p>

---

## ⚡ Technical DNA
* [cite_start]**Specialization:** Engineering autonomous **Agentic Workflows** and **Hybrid Retrieval-Augmented Generation (RAG)** systems[cite: 9, 12, 16].
* **Academic Foundation:** 2nd Year B.E. [cite_start]Computer Engineering @ **PCET’s NMIET Pune** (Current CGPA: **8.83**)[cite: 9, 56, 57].
* [cite_start]**Computation Strength:** Lead Backend & AI/ML Developer for **50+ Hackathon builds**, specializing in scalable intelligent architectures[cite: 11, 71].

---

## 🚀 Mission Logs: High-Impact Projects

### 🤖 [Intel Unnati] Hybrid Multilingual RAG
> [cite_start]*Advanced educational query engine developed under Intel Unnati Industrial Training[cite: 10, 20].*
- [cite_start]**The Core:** Implemented dense semantic retrieval using `multilingual-e5` and sparse `TF-IDF` search[cite: 26, 32].
- [cite_start]**Vector Ops:** Managed embeddings via **ChromaDB** with **MMR (Maximal Marginal Relevance)** re-ranking for grounded answer precision.
- [cite_start]**Interface:** Built multimodal pipelines supporting text, **OCR (OpenCV + Tesseract)**, and **Voice (Whisper)**[cite: 27, 33].



### 🌾 AGRIXAI: Agentic Agriculture System
- [cite_start]**Intelligence Layer:** Coupled **YOLOv8** computer vision with a **LangChain Agent** for autonomous strategic planning[cite: 40].
- [cite_start]**Knowledge Base:** Built a **FAISS** vector store from government data to provide fault-tolerant farming recommendations[cite: 41, 42].

### 🛡️ SENTINEL-X: Cyber-Threat Prediction
- [cite_start]**Logic:** Behavioral-based system predicting threats via system log analysis[cite: 51].
- [cite_start]**Validation:** Simulated attacks using Kali Linux tools (**Nmap/Hydra**) to tune anomaly detection accuracy[cite: 52, 53].

---

## 🛠️ The Tech Stack (Arsenal)

### 🧠 Intelligence & Models
[cite_start]`LangChain` `LlamaIndex` `HuggingFace` `TensorFlow` `PyTorch` `Mistral` `Gemini AI` `Ollama` [cite: 15, 16, 28, 48]

### 🏗️ Infrastructure & Backend
[cite_start]`Python (Expert)` `FastAPI` `Node.js` `Docker` `PostgreSQL` `ChromaDB` `FAISS` [cite: 14, 15, 17, 18, 31, 40]

### 🌐 Frontend & Deployment
[cite_start]`React` `Next.js` `Tailwind CSS` `Vercel` `Render` `Railway` [cite: 15, 18, 44]

---

## 🏆 Proof of Concept (Achievements)
* [cite_start]**Finalist @ Mumbai Hacks & Project Morphus:** Ranked in the top tier of national-level AI competitions[cite: 69, 95].
* [cite_start]**AI Domain Lead (ACM NMIET):** Leading a community of 500+ students in ML and GenAI exploration[cite: 86, 88].
* [cite_start]**Technical Lead (GDG NMIET):** Architected and evaluated 10+ student teams during **Cyberathon 2026**[cite: 81, 85].
* [cite_start]**Industrial Trainee:** Selected for **Intel® Unnati 2025** to build industry-grade RAG solutions[cite: 20, 97].

---

## 📊 Performance Metrics
<p align="left">
<img src="https://github-readme-stats.vercel.app/api?username=SujalKamate&show_icons=true&theme=tokyonight&hide_border=true&bg_color=00000000" alt="Sujal's Stats" />
<img src="https://github-readme-streak-stats.herokuapp.com/?user=SujalKamate&theme=tokyonight&hide_border=true&background=00000000" alt="GitHub Streak" />
</p>

---

<p align="center">
  <img src="https://raw.githubusercontent.com/Tarikul-Islam-Anik/Animated-Fluent-Emojis/master/Emojis/Hand%20gestures/Writing%20Hand.png" width="30" /> 
  <b>Open for Collaboration on AI Research & Scalable Intelligent Systems.</b>
</p>
